...
app=FastAPI(title="AI BTC Scout V4")
...
def ema_series(a,n):
    out=[]; k=2/(n+1); e=None
    for x in a:e=x if e is None else x*k+e*(1-k); out.append(e)
    return out
...
def features(rows):
    ...
    e20,e50,e200=ema_series(c,20),ema_series(c,50),ema_series(c,200); rv=rsi_series(c); av=atr_series(rows)
    score=[]; trend=[]
    for i in range(len(rows)):
        ...
        trend.append("BULLISH" if e20[i]>e50[i]>e200[i] else ("BEARISH" if e20[i]<e50[i]<e200[i] else "MIXED/BEARISH"))
    return score,rv,av,e20,trend
...
def one_test(rows,threshold):
    score,rv,av,e20,tr=features(rows); cash=START; pos=None; trades=wins=0; fees=0.; peak=START; dd=0.; pnls=[]
    for i in range(200,len(rows)):
        ...
        if pos and (p<=pos["stop"] or p>=pos["target"]):
            ...
            cash+=pos["qty"]*ep-fee; fees+=fee; trades+=1; wins+=net>0; pnls.append(net); pos=None
        if not pos and score[i]>=threshold and p>e20[i]:
            rc=max(.01,cash*RISK); dist=max(av[i]*STOP_ATR,p*.001); q=rc/dist; ef=q*p*FEE
            if q*p+ef<=cash:cash-=q*p+ef; fees+=ef; pos={"entry":p,"qty":q,"stop":p-dist,"target":p+dist*RR,"entry_fee":ef}
        eq=cash+(pos["qty"]*p if pos else 0); peak=max(peak,eq); dd=max(dd,(peak-eq)/peak*100)
    end=cash+(pos["qty"]*rows[-1][4] if pos else 0)
    return {"threshold":threshold,"ending_equity":end,"return_pct":(end/START-1)*100,"trades":trades,"wins":wins,"losses":trades-wins,"win_rate_pct":wins/trades*100 if trades else 0,"max_drawdown_pct":dd,"fees":fees,"avg_trade":sum(pnls)/len(pnls) if pnls else 0,"open_position":bool(pos)}
async def research(days):
    rows=await history(days)
    if len(rows)<250:raise RuntimeError("Not enough candles.")
    return {"days":round((rows[-1][0]-rows[0][0])/86400000,1),"candles":len(rows),"tests":[one_test(rows,t) for t in THRESHOLDS]}
...
async def scan():
    try:
        rows=await klines(220); s,rv,av,e20,tr=features(rows); i=len(rows)-1; p=rows[-1][4]
        ...
        if not state["position"] and s[i]>=75 and p>e20[i]:
            ...
            if q*p+ef<=state["cash"]:
                state["cash"]-=q*p+ef; state["fees"]+=ef; state["position"]={"entry":p,"qty":q,"stop":p-dist,"target":p+dist*RR,"entry_fee":ef}
...
async def home():
 return HTMLResponse("""<!doctype html><meta name=viewport content="width=device-width,initial-scale=1"><title>AI BTC Scout V4</title><style>body{background:#080b10;color:#eee;font:16px Arial;max-width:950px;margin:auto;padding:20px}.c{background:#121821;border:1px solid #293545;border-radius:20px;padding:20px;margin:14px 0}.g{display:grid;grid-template-columns:1fr 1fr;gap:14px}.b{font-size:28px;font-weight:bold}button{padding:12px;border:0;border-radius:10px;margin:3px}@media(max-width:600px){.g{grid-template-columns:1fr}table{font-size:13px}}</style><div class=c><h1>ð¤ AI BTC Scout V4</h1><p>BTC/USDT Â· 5-minute Â· PAPER ONLY</p><h2 id=s>WAIT</h2><div class=b id=p>$--</div></div><div class=g><div class=c>AI score<div class=b id=sc>--</div></div><div class=c>Trend<div class=b id=t>--</div></div><div class=c>RSI<div class=b id=r>--</div></div><div class=c>Equity<div class=b id=e>$20.00</div></div></div><div class=c><h2>V4 Strategy Research</h2><p>Same 90-day candles, five score thresholds. This is a research comparison, not a prediction.</p><button onclick=go()>Run 90-day research</button><div id=out>Ready.</div></div><div class=c>Paper testing only. No exchange orders or API keys. Historical results do not guarantee future performance.</div><script>
...
async function go(){out.textContent='Running 90-day research...';let x=await(await fetch('/api/research?days=90')).then(r=>r.json());if(x.error){out.textContent=x.error;return}let h='<p>Period: '+x.days+' days Â· Candles: '+x.candles.toLocaleString()+'</p><table border=1 cellpadding=8><tr><th>Score</th><th>End $</th><th>Return</th><th>Trades</th><th>Wins</th><th>Losses</th><th>Win rate</th><th>Drawdown</th><th>Fees</th><th>Avg trade</th></tr>';for(let z of x.tests)h+='<tr><td>â¥'+z.threshold+'</td><td>$'+z.ending_equity.toFixed(2)+'</td><td>'+z.return_pct.toFixed(2)+'%</td><td>'+z.trades+'</td><td>'+z.wins+'</td><td>'+z.losses+'</td><td>'+z.win_rate_pct.toFixed(1)+'%</td><td>'+z.max_drawdown_pct.toFixed(2)+'%</td><td>$'+z.fees.toFixed(4)+'</td><td>$'+z.avg_trade.toFixed(4)+'</td></tr>';out.innerHTML=h+'</table>'}q();setInterval(q,60000)</script>""")
